...
// Fallback when API key is missing or rate limited
const generateLocalFallback = (query, matchedDiseases, lang) => {
  if (matchedDiseases.length === 0) {
    return lang === "hi"
      ? "एआई सेवा वर्तमान में अनुपलब्ध है। इस समय उत्तर देने में असमर्थ।"
      : "AI feature is currently unavailable. Unable to generate response at the moment.";
  }

  const d = matchedDiseases[0];
  if (lang === "hi") {
    return `एआई सेवा वर्तमान में अनुपलब्ध है। इस समय उत्तर देने में असमर्थ। (डेटाबेस रिप्लाई): **${d.name}** के संबंध में प्राचीन आयुर्वेदिक सिद्धांतों के अनुसार निम्नलिखित सलाह दी जाती है:
- **उपयोगी जड़ी-बूटियाँ:** ${d.herbs.join(", ")}
- **आयुर्वेद उपचार:** ${d.ayurvedicUpchar.join("\n")}
...
*अस्वीकरण: यह जानकारी केवल शैक्षिक उद्देश्यों के लिए है और पेशेवर चिकित्सा सलाह का विकल्प नहीं है।*`;
  ...
  return `AI feature is currently unavailable. Unable to generate response at the moment. (Local Database Search): Based on ancient Ayurvedic guidelines for **${d.name}**, here are some wellness insights:
  ...
  try {
    ...
    if (checkEmergency(query)) {
      return res.status(200).json({
        reply:
          lang === "hi"
            ? "यह एक आपातकालीन स्थिति हो सकती है। कृपया तुरंत अपने नजदीकी अस्पताल या आपातकालीन चिकित्सा सेवाओं से संपर्क करें। आयुर्वेदिक उपचार आपातकालीन स्थितियों का विकल्प नहीं हैं।"
            : "This query suggests a potential medical emergency. Please contact your local emergency services (like 911 or emergency healthcare providers) immediately. Ayurvedic remedies are not substitutes for acute emergency care.",
        isEmergency: true,
    ...
    // Search local database for context
...
`;
    ...
    let aiText = null;
    try {
      aiText = await generateGeminiResponse(prompt);
    } catch (err) {
      console.error("Gemini service execution failed:", err.message);
    }

    // Return response or fallback if it fails
    if (aiText) {
      return res.status(200).json({
        reply: aiText,
        isEmergency: false,
    ...
    // Safe fallback
    console.warn("AI response is unavailable. Using safe fallback response.");
